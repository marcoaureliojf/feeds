Framework7.prototype.plugins.feeds = function (app) {
    ...
    var Feeds = function (container, params) {
        ...
        var listTemplate = 
            '<ul>' +
                '{{#each items}}' +
                    '<li class="ft-movielist-item swipeout">' +
                        '<a href="#" class="item-link feeds-item-link" data-index="{{@index}}">' +
                            ...
                            '<div class="swipeout-actions-right">' +
                                '<a href="#" class="swipeout-delete" data-confirm="Tem certeza que deseja excluir?" data-confirm-title="Excluir?">Excluir</a>' +
                            '</div>' +
                        '</a>' +
                    '</li>' +
                '{{/each}}' +
        ...
        '<div class="page feeds-page ' + (navbarLayout === 'fixed' ? 'navbar-fixed' : '') + '" data-page="feeds-page-{{index}}">' +
            ...
            '<div class="page-content">' +
                ...
                '</div>' +
                            '<div class="content-block">' +
                                '<div class="content-block-inner">{{description}}</div>' +
                                '<div class="col-50">' +
                                    '<a href="{{link}}" class="external button" target="_blank">Leia Mais</a><br>' +
                                '</div>' +
                            '</div>' +
            '</div>' +
        '</div>';
        ...
        f.loadFeed = function (refresh) {
            ...
            $.get(f.params.url, function (response) {
                ...
                f.data = {
                    ...
                    description: channel.children('description').text().replace('<![CDATA[', '').replace(']]>', '').replace(/(<([^>]+)>)/gi, ''),
                ...
                items.each(function (index, el) {
                    ...
                    var itemData = {
                        ...
                        description: item.children('description').text().replace('<![CDATA[', '').replace(']]>', '').replace(/(<([^>]+)>)/gi, ''),
